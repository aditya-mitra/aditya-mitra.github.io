context('footer', () => {
	beforeEach(() => {
		cy.visit('/');
		cy.get('#findMe').scrollIntoView();
	});

	const getDimensions = (element) => {
		const win = element.ownerDocument.defaultView;

		const height = win.getComputedStyle(element).getPropertyValue('height');

		const width = win.getComputedStyle(element).getPropertyValue('width');
		...
		it('has the github url', () => {
			cy.get('@builtWith')
				...
				.should(
					'have.attr',
					'href',
					'https://github.com/aditya-mitra/aditya-mitra.github.io/'
				);
		});
