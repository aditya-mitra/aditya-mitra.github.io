import { Icon, IconButton, Link, Tag, TagLabel, TagRightIcon } from '@chakra-ui/react';
 ...
 */
export const CardItems = (codex, sourceItems, colorMode, uniqueMark, hasScaledOut) => {
	let bgStyles = placeholders[codex] || placeholders['default'];
	...
	return sourceItems.map((sourceItem, idx) => {
		const { id } = sourceItem;
		const imgSrc = hasScaledOut === true ? sourceItem.imgSrc : null;
		return (
			<div key={idx}>
				...
				<div className={`wrapper ${colorMode === 'dark' ? 'dark' : null}`}>
					<div
						...
						<div className="imageContainer" style={bgStyles}>
							<img
								src={imgSrc}
 ...
 */
export const RadioBullets = (sourceItems, currentItem, totalItems, displayCard) =>
	totalItems > 1
		? sourceItems.map((sourceItem, idx) => {
				...
				return (
					<div key={idx}>
						...
						<label htmlFor={`tapInput${id}`} id={`tap${id}`} className="tap">
							<input
								type="radio"
								id={`tapInput${id}`}
								value={id}
