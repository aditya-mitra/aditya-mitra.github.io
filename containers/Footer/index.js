...
export default function Footer() {
	...
	const highlightLink = (event, extend = 20) => {
		...
		highlighter.style.transform = `translate(${coords.left - extend / 2}px,${
			coords.top - extend / 2
		}px)`;
	};

	const finds = constants.map((constant) => (
		<Link key={constant.link} isExternal href={constant.link} zIndex={50}>
			<Icon
				onMouseEnter={(event) => highlightLink(event)}
				as={constant.icon}
				color={constant.colour}
				boxSize={8}
				margin="0 1rem"
				position="relative"
				order={4}
			/>
			<span className={invisibleTextClass}>{constant.provider}</span>
		</Link>
	));
			...
			<footer className={`footer ${colorMode === 'dark' ? 'dark' : null}`}>
				...
				<div className="built-with">
					...
					<Link
						...
						<span onMouseEnter={(event) => highlightLink(event, 5)} className="built">
							NextJS
						</span>
					</Link>
				</div>
